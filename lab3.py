...
def mlParams(X, labels, W=None):
    ...
    # ==========================
    for jdx, classname in enumerate(classes):
        idx = np.where(labels==classname)[0] # Or more compactly extract the indices for which y==class is true,
        x = X[idx,:]
        w = W[idx,:]
        mu[jdx] = np.sum(x * w, axis=0) / np.sum(w)
        sigma[jdx] = np.diag(np.sum(w * (x - mu[jdx]) ** 2, axis=0) / np.sum(w))
    # ==========================
...
def classifyBayes(X, prior, mu, sigma):
    ...
    for idx in range(Npts):
        for classname in range(Nclasses):
            one = -(1/2)*np.log(np.linalg.det(sigma[classname]))
            two = -(1/2)*np.dot(np.dot(X[idx] - mu[classname], np.linalg.inv(sigma[classname])), np.transpose(X[idx] - mu[classname]))
            three = np.log(prior[classname])
            logProb[classname][idx] = one + two + three 
    # ==========================
